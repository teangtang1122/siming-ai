...
> **当前源码版本 3.3.4** 修复同一作品并发或重试生成章节时，迟到草稿替换当前草稿、随后无法保存且继续写作被阻塞的问题。PC、Gateway 与 Android 现在共同遵循“每部作品仅一个作者可见待保存草稿”的确定性状态约束，并会自动释放已对应正式正文的旧草稿。完整变化见 [3.3.4 发布说明](docs/release-notes-3.3.4.md)。
