...
- 打开“本地 AI”，让墨枢检测 CPU、内存、显卡和显存，并安装推荐的 GGUF 本地模型。
...
- 普通写作与建档：优先配置一个稳定的全局默认模型，例如 Claude Code、Codex、DeepSeek、OpenAI、通义千问或兼容 OpenAI API 的服务。
- 本地模型：适合离线草稿、辅助整理和低成本尝试；结构化建档、新书立项等任务建议先用 4B/8B 保守模型验证，复杂任务再切换更强模型。
- 外部 Agent/CLI：适合长篇资料读取、批量建档和复杂写作流程；读取优先走本地作品镜像，写入必须通过 Moshu MCP 工具。
- 模型路由：显式选择模型优先；未显式选择时跟随系统全局默认/API/CLI；只有明确指定本地任务模型时才用本地运行时覆盖。
- 发布前：同步 README、`backend/app/version.py`、`frontend/package.json`，并运行后端测试、前端构建和打包验证。
...
本版同步仓库当前版本状态，补齐项目管理、发布节奏和反馈入口，让墨枢后续开发从“连续修功能”进入“按任务稳定推进”的阶段：

- README 顶部版本说明同步到当前代码版本 `2.5.6`，避免用户看到旧的 2.5.4 说明后误判最新状态。
- 新增 `docs/roadmap.md`，按 P0-P4 拆分核心稳定、发布体验、产品可用性、本地模型、社区协作等阶段目标。
- 新增 `docs/project-management.md`，明确 issue、PR、主分支保护、开发检查清单和发布检查清单。
- 新增 Bug 反馈和功能建议 issue 模板，方便收集运行、建档、写作、模型调用和打包问题。
- `main` 作为发布基线应启用分支保护，后续代码、文档和发布变更默认通过 issue -> 分支 -> PR -> 合并推进。

### 2.5.5：新书生成与候选校验加固

本版重点处理模型输出不稳定时的新书生成和建档候选质量问题，减少“看似成功但写入脏数据”的情况：

- 新书生成接受部分模型分支成功：多路候选中至少 1 个结构合格的原创方案即可返回，不再因为其他分支失败而整轮丢弃。
- 本机 CLI 规划模型改为串行调用，并根据 CLI/本地模型路径调整超时和输出 token 上限，降低并发调用导致的失败概率。
- 蓝图结构质量闸门更贴合真实原创方案：允许紧凑但有效的方案通过，同时要求标题、主角、核心冲突、世界观和章节推进具备基本内容。
- 建档候选写入前会跳过未命名角色、空壳角色状态、无标题世界观、缺少事件描述的时间线等无效候选，避免生成“未命名角色/未命名设定”。
- 世界观写入要求标题和内容同时有效；外部 Agent 保存候选时会把跳过原因返回为 warning，便于定位模型输出问题。
- 前端在模型没有生成可用原创方案时会显示尝试模型、provider 和失败原因，不再静默用模板内容冒充模型结果。

### 2.5.4：模型路由与建档稳定性修复

本版重点修复新书生成和作品建档在 CLI/本地模型混用时的路由、重复和诊断问题：

- 作品建档现在按“显式选择模型 > 系统全局默认/API/CLI > 明确选择的本地任务模型 > 无全局默认时任务模型兜底”的顺序解析模型，避免旧的 qwen3-14b 任务设置绕过 Claude Code CLI 并启动显卡本地模型。
- 新书立项、问答继续生成、跳过问题、重新生成和方案调整都会传递当前/全局默认模型；后端三路候选生成允许部分成功并重试失败分支，至少 1 个结构合格的原创方案即可返回，不再用模板内容冒充模型生成。
- 本地模型中心的任务模型可以清空并回到“跟随全局默认/API/CLI”，任务模型文案明确它是本地运行时的显式覆盖。
- 建档候选解析继续支持 `character_state_update`，兼容 `character_state` 等旧别名，并能从缺失 `type` 的角色、关系、世界观字段推断候选类型；无法推断时会显示字段和片段，不再出现空白 `未知 type:`。
- 建档任务响应和状态卡增加 effective model、来源和 provider 诊断，便于确认实际运行的是 Claude CLI、云端 API 还是本地 llama.cpp。
- Windows 杀毒软件若对打包版误报，常见原因是未签名 PyInstaller/内嵌 Python 运行时会启动本地服务、CLI 或模型进程；Moshu 不会把模型权重打包进 exe，也不会主动上传用户作品数据。

### 2.5.3：原创生成与建档稳定性修复

本版修复新书创建和作品建档在弱模型/外部 CLI 下的重复与解析问题：

- 混合/内部 LLM 新书创建不再在模型失败时把固定模板当作生成结果返回，避免多名用户拿到高度相似的方案；需要模板草稿时仍可明确选择模板模式。
- 作品建档第二阶段在本地模型输出不可解析候选时会暂停当前章节并提示换模型或重跑，不再自动生成模板化章节摘要和大纲候选。
- 外部 Agent/CLI 建档候选保存入口会归一化 `character_state`、`new_character`、空 type 但字段可推断等常见输出，并在入库前按章节和语义签名去重。
- 本地模型推荐更保守，14B 只推荐给明显高配机器，普通设备默认优先 4B/8B，降低建档时的资源压力。

### 2.5.2：本地模型建档与新书创建稳定性修复

本版重点修复小型本地模型在结构化任务中的稳定性问题：

- 新书创建会更精确识别问答槽位，主角身份/能力/处境不再误归到力量体系，也不会反复追问同一个已回答问题。
- 本地 llama.cpp 初稿默认走确定性模板兜底，模型增强结果必须通过结构质量闸门，避免 4B 模型生成低分、跑题或重复方案。
- 作品建档候选事实入库前会按章节、类型、目标、标题和证据去重，避免本地模型反复输出同一组 `worldbuilding_timeline` 后堆出重复 pending 项。
- 模型中心会按模型清单保存任务上下文长度，qwen3-4b-q4 等 32K 模型会真正按更长上下文请求启动；启动失败时仍会自动降级。
