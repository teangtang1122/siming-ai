            ...
            with engine.begin() as connection:
                connection.execute(
                    text(
                        ...
                        "(id VARCHAR(36) PRIMARY KEY, title VARCHAR(200))"
                    )
                )
                connection.execute(
                    text(
                        "INSERT INTO projects (id, title) "
                        "VALUES ('legacy-project', 'Legacy Story')"
                    )
                )
...
def test_unknown_database_enters_read_only_recovery_without_mutation():
    with TemporaryDirectory() as temp_dir:
        ...
        try:
            with engine.begin() as connection:
                connection.execute(
                    text("CREATE TABLE unrelated_data (id INTEGER PRIMARY KEY)")
                )
                connection.execute(
                    text("INSERT INTO unrelated_data (id) VALUES (1)")
                )
            ...
            with engine.connect() as connection:
                assert connection.execute(
                    text("SELECT COUNT(*) FROM unrelated_data")
                ).scalar_one() == 1
        finally:
            engine.dispose()
