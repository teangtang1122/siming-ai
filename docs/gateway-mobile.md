...
也可以直接使用与桌面端和 APK 相同版本的镜像，例如当前源码版本对应 `ghcr.io/teangtang1122/siming-ai-gateway:3.3.6`。生产部署应固定完整版本号，不要仅依赖 `latest`。默认映射宿主机 8000 端口，数据写入 Docker 卷 `siming-gateway-data`。首次打开管理页时输入 `SIMING_GATEWAY_BOOTSTRAP_KEY`；成功后服务器写入 12 小时 HttpOnly、SameSite=Strict 会话 Cookie，口令不会进入浏览器存储。当前同步协议仍为 v1。
