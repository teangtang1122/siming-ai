...
当前验证记录：合入 v3.3.6 后，后端架构、PromptSpec/预算和导入边界均通过；完整后端为 1798 passed，2 个代理用例清除测试环境代理后通过，剩余 3 个仅因当前容器会提前终止短生命周期子进程而失败。前端 lint、全量 221 项测试、架构、重复度、OpenAPI 和生产构建均通过；移动端/PC 的 27 项能力契约和 Python 侧移动契约测试此前已通过。Android Gradle 测试仍因当前环境无法下载 Gradle 8.9 而未执行，交由联网的 GitHub Actions 发布门禁完成。
